...
export function PersonCard({ person, sources, marriages, expanded, onToggle }: Props) {
  ...
  return <article className={`person-card ${expanded ? 'is-expanded' : ''}`}>
    ...
    {expanded && <div className="person-card__details">
      ...
      {!!person.biography?.stories?.length && <section>{person.biography.stories.map((story, i) => <div className="story" key={i}><div className="story__heading"><h3>{story.title}</h3>{story.confidence && <span className={`confidence confidence--${story.confidence}`}>{confidenceLabel[story.confidence]}</span>}</div><p>{story.text}</p></div>)}</section>}
